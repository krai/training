...
def main(args):
    ...
    dummy_input = torch.randn(args.batch_size, 3, args.image_size[0], args.image_size[1], requires_grad=True)
    # out = model_without_ddp(dummy_input)
    # print("Debug Out:", out)

    # Export the model
    torch.onnx.export(model_without_ddp,         # model being run
         dummy_input,       # model input (or a tuple for multiple inputs)
         "/results/ImageClassifier.onnx",       # where to save the model
